...
const settings = PRIVATERED.settings;
...
module.exports = {
    get: function (broker, port, clientId, username, password, will) {
        const id = "[" + (username || "") + ":" + (password || "") + "][" + (clientId || "") + "]@" + broker + ":" + port;
        if (!connections[id]) {
            connections[id] = function () {
                const uid = (1 + Math.random() * 4294967295).toString(16);
                let client = mqtt.createClient(port, broker);
                client.uid = uid;
                ...
                var obj = {
                    ...
                    subscribe: function (node, topic, qos, callback) {
                        const _callback = function (mtopic, mpayload, mqos, mretain) {
                    ...
                    unsubscribe: function (node, topic) {
                        let subs_to_delete = subscriptions.filter(sub => (sub.topic === topic && sub.node.id === node.id))
                ...
                client.on('connect', function () {
                    if (client) {
                        ...
                        connecting = false;
                        subscriptions.forEach((value, idx, [s]) => {
                            const topic = subscriptions[s].topic;
                            const qos = subscriptions[s].qos;
                            const callback = subscriptions[s].callback;
                            client.subscribe(topic, qos);
                        });
